...
**Three-stage confirmation (the default chat flow; optional Confirm UI mirrors it).** The confirmation fields split into a dependency order, confirmed in three passes:
...
> **Default presentation surface — reusable chat contact sheets.** Show `assets/contact-sheets/ppt_visual_styles.png` and, when professional analysis is considered, `assets/contact-sheets/analysis_visual_styles.png`; first present the four domains in `assets/contact-sheets/analysis_domains.md`. After the user chooses architecture / interior / landscape / planning, read that domain's complete ordered list from `analysis_types.json`, state the total, list every stable id and user-facing name, and ask whether to generate all items or which ids to exclude. Stay at the same blocking decision until the user confirms; persist the full domain list minus only explicit exclusions. Never derive a smaller set from the sources. You still author everything — generative fields (color palette, CJK + Latin typography, generated-image style) each carry **≥3 distinct candidates**, and the deck's visual style carries a ≥3-style personality spectrum. Honest-shortfall exception: if the constraints genuinely cannot yield 3 non-conflicting options, present the smaller set and say why. When the user explicitly asks for an interactive browser editor, write `<project>/confirm_ui/recommendations.json` and launch the optional Confirm UI per [SKILL.md Step 4](../SKILL.md); otherwise the chat reply is authoritative and no local server is started.
...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a compact, machine-readable form of the color / typography / icon / image / **page_rhythm** / **page_charts** and route-specific PowerPoint structure decisions above. Within the PowerPoint structure portion, free-design and brand-only routes write only `pptx_structure.mode: flat`; deck/layout template routes additionally write `page_layouts`, `page_pptx_layouts`, `pptx_masters`, and `pptx_layouts`. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **pptx_structure is mandatory**: Free-design and brand-only routes write `mode: flat`. Deck/layout template routes write `mode: structured` plus `template_adherence: strict|adaptive`. Do not write legacy `baseline`, `template`, `preserve`, `layout_strategy`, or Layout-kind rows into a new project.
