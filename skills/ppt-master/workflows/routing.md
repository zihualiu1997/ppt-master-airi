...
|---|---|---|---|---|---|---|
| New or restructured deck | Any new deck, or any source deck whose page count/order/outline may change | [`airi-workflow-wizard`](./airi-workflow-wizard.md), then main Executor/export steps | Early page-count confirmation or direct SVG generation | Project can be initialized; source may be uploaded in the wizard | schema-v4 selection, explicit professional-analysis decision, current content inventory, confirmed sourced page outline, exported PPTX | Stop before Executor while any generation gate is false |
...
| Source material can be reworked into a new story | PDF/DOCX/URL/Markdown/text/conversation content, or PPTX treated as content | Main `SKILL.md` pipeline through [`airi-workflow-wizard`](./airi-workflow-wizard.md) | Direct PPTX edit workflows | Source content exists or is available in conversation | content inventory, confirmed outline, `design_spec.md`, `spec_lock.md`, `svg_output/`, exported PPTX | Stop at workflow gates when required artifacts are missing |
