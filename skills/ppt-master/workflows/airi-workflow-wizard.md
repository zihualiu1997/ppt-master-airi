...
- **True**: show the reusable numbered analysis-style contact sheet and the four entries from `assets/contact-sheets/analysis_domains.md`. Require one style, exactly one domain, the analysis library id, optional reference images, and a provider profile. After the domain is selected, read its complete ordered list from `analysis_types.json`, state the total, list every stable id and user-facing name, and ask whether to generate all items or which ids to exclude. Do not auto-filter or recommend a smaller set from the source material. Stay at this blocking decision until the user explicitly confirms all or supplies exclusions. Persist the full domain list minus only those exclusions; an empty runtime selection resolves to the full domain for compatibility. Generate only through `analysis_library.py` + `image_gen.py`. Types without measured source data remain conceptual and must not invent measurements, simulations, performance claims, or compliance conclusions.

Write schema-v4 `workflow_selection.json` with `analysis_domain_id`, user-confirmed `analysis_item_ids`, `analysis_selection_confirmed`, and `analysis_selection_hash`. Changing style, domain, confirmed type set, reference image, provider profile, or the professional-analysis decision invalidates downstream artifacts.
